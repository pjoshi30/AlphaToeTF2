...
def create_convolutional_network():
    input_layer = tf.input_layer = tf.compat.v1.placeholder("float",
    ...
    convolution_weights_1 = tf.Variable(tf.random.truncated_normal([3, 3, 1, CONVOLUTIONS_LAYER_1], stddev=0.01))
    convolution_bias_1 = tf.Variable(tf.constant(0.01, shape=[CONVOLUTIONS_LAYER_1]))

    convolution_weights_2 = tf.Variable(
        tf.random.truncated_normal([3, 3, CONVOLUTIONS_LAYER_1, CONVOLUTIONS_LAYER_2], stddev=0.01))
    convolution_bias_2 = tf.Variable(tf.constant(0.01, shape=[CONVOLUTIONS_LAYER_2]))

    convolution_weights_3 = tf.Variable(
        tf.random.truncated_normal([3, 3, CONVOLUTIONS_LAYER_2, CONVOLUTIONS_LAYER_3], stddev=0.01))
    convolution_bias_3 = tf.Variable(tf.constant(0.01, shape=[CONVOLUTIONS_LAYER_3]))

    convolution_weights_4 = tf.Variable(
        tf.random.truncated_normal([3, 3, CONVOLUTIONS_LAYER_3, CONVOLUTIONS_LAYER_4], stddev=0.01))
    convolution_bias_4 = tf.Variable(tf.constant(0.01, shape=[CONVOLUTIONS_LAYER_4]))

    convolution_weights_5 = tf.Variable(
        tf.random.truncated_normal([3, 3, CONVOLUTIONS_LAYER_4, CONVOLUTIONS_LAYER_5], stddev=0.01))
    ...
    feed_forward_weights_2 = tf.Variable(
        tf.random.truncated_normal([FLAT_SIZE, connect_4_game_spec.outputs()], stddev=0.01))
    feed_forward_bias_2 = tf.Variable(tf.constant(0.01, shape=[connect_4_game_spec.outputs()]))

    hidden_convolutional_layer_1 = tf.nn.relu(
        tf.nn.conv2d(input=input_layer, filters=convolution_weights_1, strides=[1, 1, 1, 1], padding="SAME") + convolution_bias_1)

    hidden_convolutional_layer_2 = tf.nn.relu(
        tf.nn.conv2d(input=hidden_convolutional_layer_1, filters=convolution_weights_2, strides=[1, 1, 1, 1],
                     padding="SAME") + convolution_bias_2)

    hidden_convolutional_layer_3 = tf.nn.relu(
        tf.nn.conv2d(input=hidden_convolutional_layer_2, filters=convolution_weights_3, strides=[1, 1, 1, 1],
                     padding="SAME") + convolution_bias_3)

    hidden_convolutional_layer_4 = tf.nn.relu(
        tf.nn.conv2d(input=hidden_convolutional_layer_3, filters=convolution_weights_4, strides=[1, 1, 1, 1],
                     padding="SAME") + convolution_bias_4)

    hidden_convolutional_layer_5 = tf.nn.relu(
        tf.nn.conv2d(input=hidden_convolutional_layer_4, filters=convolution_weights_5, strides=[1, 1, 1, 1],
