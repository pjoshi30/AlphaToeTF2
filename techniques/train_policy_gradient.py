...
def train_policy_gradients(game_spec,
    ...
    actual_move_placeholder = tf.compat.v1.placeholder("float", shape=(None, game_spec.outputs()))

    input_layer, output_layer, variables = create_network()

    policy_gradient = tf.math.log(
        tf.reduce_sum(input_tensor=tf.multiply(actual_move_placeholder, output_layer), axis=1)) * reward_placeholder
    ...
    with tf.compat.v1.Session() as session:
        ...
        # This is the function that will be called for a given player's move
