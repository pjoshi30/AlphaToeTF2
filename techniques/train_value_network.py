...
def train_value_network(game_spec, hidden_nodes_reinforcement, reinforcement_network_file_path,
    ...
    error = tf.reduce_sum(input_tensor=tf.square(target_placeholder - value_output_layer))

    train_step = tf.compat.v1.train.RMSPropOptimizer(learn_rate).minimize(error)
