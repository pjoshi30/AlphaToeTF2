...
def train_policy_gradients_vs_historic(game_spec, create_network, network_file_path,
    ...
    actual_move_placeholder = tf.compat.v1.placeholder("float", shape=(None, game_spec.board_squares()))
    policy_gradient = tf.reduce_sum(input_tensor=tf.reshape(reward_placeholder, (-1, 1)) * actual_move_placeholder * output_layer)
    train_step = tf.compat.v1.train.RMSPropOptimizer(learn_rate).minimize(-policy_gradient)
