...
def train_supervised(game_spec, create_network, network_file_path,
    ...
    actual_move_placeholder = tf.compat.v1.placeholder("float", (None, game_spec.outputs()))

    error = tf.reduce_sum(input_tensor=tf.square(actual_move_placeholder - output_layer))
    ...
    correct_pred = tf.equal(tf.argmax(input=output_layer, axis=1), tf.argmax(input=actual_move_placeholder, axis=1))
